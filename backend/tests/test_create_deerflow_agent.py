...
@patch("deerflow.agents.factory.create_agent")
def test_custom_state_schema_is_preserved(mock_create_agent):
    mock_create_agent.return_value = MagicMock(name="compiled_graph")
...
@patch("deerflow.agents.factory.create_agent")
def test_supplied_middleware_is_passed_through_unchanged(mock_create_agent):
    mock_create_agent.return_value = MagicMock(name="compiled_graph")
    middleware = ViewImageMiddleware()
    original_schema = middleware.state_schema

    create_deerflow_agent(_make_mock_model(), middleware=[middleware])

    call_kwargs = mock_create_agent.call_args.kwargs
    assert call_kwargs["state_schema"] is ThreadState
    assert call_kwargs["middleware"][0] is middleware
    assert middleware.state_schema is original_schema


def test_compiled_factory_graph_uses_the_reducer_message_channel():
