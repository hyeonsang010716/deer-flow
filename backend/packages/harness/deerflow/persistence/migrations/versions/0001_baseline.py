"""baseline — DeerFlow 애플리케이션 schema의 chain root.
...
alembic을 도입한 시점에 ``Base.metadata.create_all``이 모든 DeerFlow 소유 테이블에 대해
만들어내는 schema를 담고 있다. hybrid bootstrap 전략
(``deerflow.persistence.bootstrap.bootstrap_schema``)에서는 여기의 ``upgrade()``가
**거의 실행되지 않는다**:

- 새 DB      -> ``create_all`` + ``alembic stamp head``(upgrade 실행 없음).
- 레거시 DB  -> ``alembic stamp 0001_baseline`` + ``upgrade head``
                (바로 다음 revision으로 건너뛴다. alembic은 stamp된 위치보다 엄격히
                뒤에 있는 revision만 실행하므로 baseline ``upgrade()``도 돌지 않는다).
- 버전 관리 DB -> ``upgrade head``(``alembic_version``에 있는 revision부터 이어간다.
                baseline ``upgrade()``는 DB가 마침 ``base``에 있을 때만 돈다).

따라서 baseline은 주로 **stamp 대상 + chain root** 역할을 한다. 테스트 fixture에서
``alembic upgrade base -> head``가 왕복되도록 ``upgrade()``는 ``Base.metadata``에 충실하게
유지하고 대칭을 위해 ``downgrade()``도 전부 제공하지만, 운영 경로의 정확성이 이 revision의
DDL이 ``create_all``과 바이트 단위로 일치하는지에 달려 있지는 않다.

LangGraph checkpointer 테이블(``checkpoints``, ``checkpoint_blobs``,
``checkpoint_writes``, ``checkpoint_migrations``)은 의도적으로 빠져 있다. LangGraph 소유이며
``env.py::include_object``가 제외한다.
"""
